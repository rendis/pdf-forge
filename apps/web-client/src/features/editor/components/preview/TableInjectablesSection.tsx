...
import type { Variable, TableColumnMeta } from '../../types/variables'
...
interface TableInjectablesSectionProps {
  variables: Variable[]
...
export function TableInjectablesSection({
  variables,
  values,
  onChange,
  disabled = false,
}: TableInjectablesSectionProps) {
      ...
      <CollapsibleContent className="overflow-hidden data-[state=open]:animate-collapsible-down data-[state=closed]:animate-collapsible-up">
        <div className="space-y-4 bg-muted/30 p-3 rounded-sm border border-border">
          {variables.map((variable, index) => {
            const columns = (variable.metadata?.columns as TableColumnMeta[]) ?? []
            ...
            return (
              <div key={variable.variableId}>
                <TableDataInput
                  ...
                  disabled={disabled}
                />
                {/* Separator between tables (except last) */}
